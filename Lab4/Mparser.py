...
def p_instructions(p):
    ...
    """
    if len(p) == 2:
        p[0] = AST.Instructions([p[1]], p.lineno(1))
    else:
        instr = p[1]
...
def p_print_stmt(p):
    ...
    """
    p[0] = AST.PrintStatement(p[2], p.lineno(1))
...
def p_range(p):
    ...
    """
    p[0] = AST.RangeStatement(p[1], p[3], p.lineno(1))
...
def p_if_stmt(p):
    ...
    """
    if len(p) == 6:
        p[0] = AST.IfStatement(p[3], p[5], None, p.lineno(1))
    else:
